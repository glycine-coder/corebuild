(function(root){
  ...
  function showConsent(){
    ...
    const overlay=document.createElement('div');overlay.id='answerSyncConsent';overlay.setAttribute('role','dialog');overlay.setAttribute('aria-modal','true');overlay.innerHTML=channel==='public'?'<div class="answer-sync-consent"><p class="answer-sync-consent-eyebrow">任意の回答データ提供</p><h2>回答データ保存の確認</h2><p>学習機能の改善と問題ごとの回答傾向の分析のため、回答内容、正誤、得点、試験・設問ID、回答時刻をCloudflare D1へ保存します。氏名やメールアドレスは収集しません。</p><ul><li>保存期間：受信から365日</li><li>同意しなくても、すべての問題を利用できます</li><li>詳しくは<a href="privacy.html" target="_blank" rel="noopener">回答データの取扱方針</a>をご確認ください</li></ul><div class="answer-sync-consent-actions"><button type="button" data-consent="declined">保存せず続ける</button><button type="button" data-consent="accepted" class="primary">同意して保存する</button></div></div>':'<div class="answer-sync-consent"><p class="answer-sync-consent-eyebrow">非公開・試験運用</p><h2>回答データ保存の確認</h2><p>この管理用テストでは、回答内容、正誤、得点、試験・設問ID、回答時刻を、検証専用のCloudflare D1へ保存します。氏名やメールアドレスは収集しません。</p><ul><li>保存目的：回答保存機能と集計方法の品質確認</li><li>保存区分：本番統計に含めない試験データ</li><li>同意しなくても問題は利用できます</li></ul><div class="answer-sync-consent-actions"><button type="button" data-consent="declined">保存せず続ける</button><button type="button" data-consent="accepted" class="primary">同意して続行</button></div></div>';
